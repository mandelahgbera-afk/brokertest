...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchAppUser = useCallback(async (authUser: User) => {
    try {
      // Single query: find by auth_id OR email (much faster than two sequential queries)
      ...
      if (existing) {
        // Link auth_id in background if missing — don't await, no need to block
        if (!existing.auth_id) {
          supabase.from('users').update({ auth_id: authUser.id }).eq('id', existing.id).then(() => {});
          setUser({ ...existing, auth_id: authUser.id } as AppUser);
        } else {
      ...
      // New user — insert profile and balance in parallel
      ...
      if (newUser) {
        // Create balance row in background — don't block login
