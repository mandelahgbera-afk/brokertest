...
function ChartTooltip({ active, payload }: any) {
  ...
  return (
    <div className="glass-dark rounded-xl px-3 py-2 text-xs border border-white/10 bg-card">
      <p className="text-muted-foreground">{payload[0].payload.t}</p>
      <p className="font-mono font-bold text-primary">${payload[0].value?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
    </div>
  );
}

function buildChartData(txns: any[], currentBalance: number) {
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.transactions.getByEmail(user.email, 200),
    ]).then(([bal, port, cry, recentTx, allTx]) => {
  ...
  const chartData = useMemo(() => buildChartData(allTxns, totalBalance), [allTxns, totalBalance]);
        ...
        <div className="relative">
          ...
          <div className="flex items-end gap-4 mb-4">
            <h2 className="text-4xl font-black font-mono tabular-nums">
              {loading ? <span className="text-muted-foreground/40">Loading...</span>
                : hideBalance ? '••••••'
            ...
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="v" stroke={chartColor} strokeWidth={2} fill="url(#chartGrad)" dot={false} activeDot={{ r: 4, strokeWidth: 0 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
            {cryptos.slice(0, 5).map(c => (
              <div key={c.id} className="flex flex-col gap-1 px-3 py-3 rounded-2xl bg-secondary hover:bg-secondary/80 transition-colors">
                ...
                <p className="text-sm font-mono font-bold tabular-nums">${c.price?.toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-40 gap-3">
            ...
            <Link to="/transactions" className="text-xs font-semibold text-primary hover:underline px-4 py-2 rounded-xl bg-primary/10 border border-primary/20">
